...
export function collectionHandlers({
  ...
  async function GET() {
    ...
    // Read-only demo mode: surface the bundled seed content so the admin is
    // browsable before a database is connected (mutations still return 503).
    if (!isDbConfigured()) {
      const items = (seed || []).map((s, i) => ({ _id: s.slug || s.key || `seed-${i}`, ...s }));
      return json({ ok: true, items, demo: true });
    }
